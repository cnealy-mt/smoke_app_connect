...
# -------------------------
r <- rast("data/MASSDEN/2025-12-30_MASSDEN_max_lead0.tif")
ext_r <- ext(trim(r))
bounds <- list(
...
ggsave(
  filename = "misc/test_png_bounds/test_bounds.png",
  plot = gg,
  bg = "transparent",
  dpi = 100,
  width = 6,
...
png_path <- "misc/test_png_bounds/test_bounds.png"
...
# -------------------------
montana_bounds <- list(
  SW = c(bounds[[1]][1] - 0.19, bounds[[1]][2] - 0.05),  # move south & west
  NE = c(bounds[[2]][1] - 0.19, bounds[[2]][2] + 0.05)   # move north & east
)
...
leaflet() %>%
  ...
  htmlwidgets::onRender(sprintf("
    function(el, x) {
      var map = this;
      var bounds = [[%f, %f], [%f, %f]];
      var img = L.imageOverlay('%s', bounds, {opacity: 0.8});
      img.addTo(map);
    }
  ",
  ...
  addRasterImage(r1_raster, opacity = 0.3) %>%
