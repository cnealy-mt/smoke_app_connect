  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$map_rendered, {
      forecast_datetime <- as.POSIXct(as.Date(today)) +
        lubridate::hours(layer()) + lubridate::hours(12 + offset_hours) #12 is for the 12z model run time
        #ex: 1st forecast layer = 6am when MT is -7 UTC offset; i.e., 1hr + 12 + -7 = 1 + 5 = 6am forecast
    ...
    observeEvent(layer(), {
      ...
      forecast_datetime <- as.POSIXct(as.Date(today)) +
        lubridate::hours(layer()) + lubridate::hours(12 + offset_hours)
